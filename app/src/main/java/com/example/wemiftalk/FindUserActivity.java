...
import com.example.wemiftalk.User.UserListAdapter;
    ...
    private void createChat(){
        ...
        for(UserObject mUser : userList){
            if(mUser.getSelected()){
                ...
                userDb.child(mUser.getUid()).child("chat").child(key).setValue(true); //przypisanie czatu do kontaktu, w który kliknęliśmy
            }
        }

        if(validChat){ // jesli jakikolwiek user jest wybrany to czat moze sie stworzyc
            chatInfoDb.updateChildren(newChatMap);
            userDb.child(FirebaseAuth.getInstance().getUid()).child("chat").child(key).setValue(true); //przypisanie czatu do aktywnego użytkownika
        }
